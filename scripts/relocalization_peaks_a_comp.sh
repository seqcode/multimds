...
if [ -e peaks_a_comp_filtered.bed ]
	then
		rm peaks_a_comp_filtered.bed
fi
...
do
	...
	python relocalization_peaks_multimds.py $CHROM
	...
	cat ${CHROM}_A_noncomp_peaks_filtered.bed >> peaks_a_comp_filtered.bed
done
...
bedtools subtract -a A_compartment_${RES_KB}kb.bed -b peaks_a_comp_filtered.bed > A_background.bed
