...
do
	#convert to BED
	if [ ! -s hic_data/${OUTPUT_PREFIX}-G1E_${CHROM}_40kb.bed ]
		then
	...
	#bin BED
	if [ ! -s hic_data/${OUTPUT_PREFIX}-G1E_${CHROM}_${RES_KB}kb.bed ]
		then
...
#convert to BED
if [ ! -s hic_data/${OUTPUT_PREFIX}-G1E_X_40kb.bed ]
	then	
...
#bin BED
if [ ! -s hic_data/${OUTPUT_PREFIX}-G1E_X_${RES_KB}kb.bed ]
	then
