...
do
	#convert to BED
	if [ ! -e hic_data/${OUTPUT_PREFIX}-G1E_${CHROM}_40kb.bed ]
		then
			cat GEO/40kb_${INPUT_PREFIX}G1E-ER4_uninduced_iced_chr$CHROM.matrix | awk -v chrom=$CHROM '{print "chr"chrom"\t"$1"\t"$1+40000"\tchr"chrom"\t"$2"\t"$2+40000"\t"$3}' > hic_data/${OUTPUT_PREFIX}-G1E_${CHROM}_40kb.bed
	fi

	#bin BED
	if [ ! -e hic_data/${OUTPUT_PREFIX}-G1E_${CHROM}_${RES_KB}kb.bed ]
		then
...
#convert to BED
if [ ! -e hic_data/${OUTPUT_PREFIX}-G1E_X_40kb.bed ]
	then	
		cat GEO/40kb_${INPUT_PREFIX}G1E-ER4_uninduced_iced_chrX.matrix | awk '{print "chrX\t"$1"\t"$1+40000"\tchrX\t"$2"\t"$2+40000"\t"$3}' > hic_data/${OUTPUT_PREFIX}-G1E_X_40kb.bed
fi
	
#bin BED
if [ ! -e hic_data/${OUTPUT_PREFIX}-G1E_X_${RES_KB}kb.bed ]
	then
