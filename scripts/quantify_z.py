	...
	for line in infile:
		...
		for chrom in chroms:
			...
			if os.path.isfile(path1) and os.path.isfile(path2):
				...
				#np.savetxt("{}_{}_comps.txt".format(cell_type1, chrom), compartments1)
				#np.savetxt("{}_{}_comps.txt".format(cell_type2, chrom), compartments2)

				#compartments1 = np.loadtxt("{}_{}_comps.txt".format(cell_type1, chrom))
				#compartments2 = np.loadtxt("{}_{}_comps.txt".format(cell_type2, chrom))
...
y_start = min((min(x_length_fractions), min(y_length_fractions), min(z_length_fractions))) -y_int_size/5.
y_end = max((max(x_length_fractions), max(y_length_fractions), max(z_length_fractions))) + y_int_size/5.
