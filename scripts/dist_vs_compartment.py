...
for j, chrom in enumerate(chroms):	
	...
	os.system("python ../multimds.py -m {} -N {} -o /data/drive1/hic_data/ {} {}".format(midpoints[j]*10**6, partition_nums[j], path1, path2))

	#load structures
	structure1 = dt.structure_from_file("/data/drive1/hic_data/{}_{}_{}kb_structure.tsv".format(cell_type1, chrom, res_kb))	
	structure2 = dt.structure_from_file("/data/drive1/hic_data/{}_{}_{}kb_structure.tsv".format(cell_type2, chrom, res_kb))
	...
	#compartments
	compartments1 = ca.get_compartments(mat1, structure1, "binding_data/Gm12878_{}_{}kb_active_coverage.bed".format(chrom, res_kb))
	compartments2 = ca.get_compartments(mat2, structure2, "binding_data/K562_{}_{}kb_active_coverage.bed".format(chrom, res_kb))
...
fig, ax = plt.subplots()
rects1 = ax.bar(ind, contacts_pearson_rs, width, color="c")
rects2 = ax.bar(ind + width, contacts_spearman_rs, width, color="gray")
#rects3 = ax.bar(ind + 2*width, dists_pearson_rs, width, color="m")
#rects4 = ax.bar(ind + 3*width, dists_spearman_rs, width, color="y")
rects3 = ax.bar(ind + 2*width, multimds_z_rs, width, color="r")
