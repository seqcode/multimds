...
#	then
		...
		if [ $NF -eq 3 ]
			then
				bedtools map -a $PREFIX"_sorted".bed -b /data/drive1/test/archive/multimds/scripts/mappability_sorted.bed -o mean -c 4 | awk '$4 >= 0.75 {print $1"\t"$2"\t"$3}' > $PREFIX"_filtered".bed	#filter out mappability < 0.75
		elif [ $NF -eq 4 ]
			then
				bedtools map -a $PREFIX"_sorted".bed -b /data/drive1/test/archive/multimds/scripts/mappability_sorted.bed -o mean -c 4 | awk '$5 >= 0.75 {print $1"\t"$2"\t"$3"\t"$4}' > $PREFIX"_filtered".bed	
		fi
#fi
